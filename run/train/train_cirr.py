...
class CIRRTrainer(BaseTrainer):
    ...
    def validate(self):
        ...
        group_recall_at1, _, _, _, recall_at5, _, _ = compute_cirr_val_metrics(
...
if __name__ == "__main__":
    ...
    parser.add.add_argument("--num-workers", type=int, default=4)
    ...
    parser.add.argument("--lr", default=4e-5, type=float, help="Combiner learning rate")
    parser.add.argument("--batch-size", default=1024, type=int)
    parser.add.argument("--patch-num", default=13, type=int)
    parser.add.argument("--clip-bs", default=4, type=int, help="Batch size during CLIP feature extraction")
    parser.add.argument("--validation-bs", default=1024, type=int, help="Batch size during validation")
    parser.add.argument("--validation-frequency", default=3, type=int, help="Validation frequency expressed in epochs")
    parser.add.argument("--print-frequency", default=100, type=int)
    parser.add.argument("--target-ratio", default=1.25, type=float, help="TargetPad target ratio")
    parser.add.argument("--transform", default="targetpad", type=str, help="Preprocess pipeline")
    parser.add.argument("--save-training", dest="save_training", action='store_true', help="Whether save the training model")
    parser.add.argument("--save-best", dest="save_best", action='store_true', help="Save only the best model during training")
    ...
    trainer = CIRRTrainer(**training_hyper_params)
    trainer.train()
