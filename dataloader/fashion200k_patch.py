...
_DEFAULT_FASHION_IQ_DATASET_ROOT = '/mnt/vision_retrieval/chenyanzhe/fashion-200k'
_DEFAULT_FASHION_IQ_VOCAB_PATH = '/mnt/vision_retrieval/chenyanzhe/fashion-200k/fashion200k_vocab.pkl'
...
def caption_post_process(s):
    return s.strip().replace('.', 'dotmark').replace('?', 'questionmark').replace('&', 'andmark').replace('*',
                                                                                                          'starmark')
...
class Fashion200kDataset(AbstractBaseFashion200kDataset):
    ...
    def __getitem__(self, idx, use_transform=True):
        ...
        ref_patch_path = ref_img_path.replace("women", "local_features") + ".pth"
        tar_patch_path = targ_img_path.replace("women", "local_features") + ".pth"
        ref_patch_feat = torch.load(ref_patch_path)
        tar_patch_feat = torch.load(tar_patch_path)

        return reference_img, target_img, modifier, len(modifier), ref_id, targ_id, ref_patch_feat, tar_patch_feat
...
class Fashion200kTestDataset(AbstractBaseFashion200kDataset):
    ...
    def __getitem__(self, idx, use_transform=True):
        ...
        img_path = os.path.join(self.root_path, img['file_path'])
        patch_path = img_path.replace("women", "local_features") + ".pth"
        patch_feat = torch.load(patch_path)

        return img_id, target_img, patch_feat
...
class Fashion200kTestQueryDataset(AbstractBaseFashion200kDataset):
    ...
    def __getitem__(self, idx, use_transform=True):
        ...
        return ref_img, ref_id, modifier, targ_id, len(modifier), ref_patch_feat
...
class Fashion200kDatasetRN50(AbstractBaseFashion200kDataset):
    def __init__(self, root_path=_DEFAULT_FASHION_IQ_DATASET_ROOT, clothing_type=None, split='train',
                 img_transform=None, text_transform=None, id_transform=None):
        super().__init__(root_path, split, img_transform, text_transform)
        self.root_path = root_path
    ...
    def __getitem__(self, idx, use_transform=True):
        ...
        ref_patch_path = ref_img_path.replace("women", "fashion200k_13_patch") + ".pth"
        tar_patch_path = targ_img_path.replace("women", "fashion200k_13_patch") + ".pth"
        ref_patch_feat = torch.load(ref_patch_path)
        tar_patch_feat = torch.load(tar_patch_path)
...
class Fashion200kTestDatasetRN50(AbstractBaseFashion200kDataset):
    def __init__(self, root_path=_DEFAULT_FASHION_IQ_DATASET_ROOT, clothing_type=None, split='test',
                 img_transform=None, text_transform=None):
        super().__init__(root_path, split, img_transform, text_transform)
        if split == 'val':
    ...
    def __getitem__(self, idx, use_transform=True):
        ...
        img_path = os.path.join(self.root_path, img['file_path'])
        patch_path = img_path.replace("women", "fashion200k_13_patch") + ".pth"
        patch_feat = torch.load(patch_path)
...
class Fashion200kTestQueryDatasetRN50(AbstractBaseFashion200kDataset):
    def __init__(self, root_path=_DEFAULT_FASHION_IQ_DATASET_ROOT, clothing_type=None, split='test',
                 img_transform=None, text_transform=None):
        if split == 'val':
            split = 'test'
