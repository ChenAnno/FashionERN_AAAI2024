...
class DVR_module(nn.Module):
    def __init__(self, feature_dim=640, device=None):
        ...
        self.transformer_before = PlusModel(
            feature_dim=feature_dim, device=device, layers=2
        ).to(device)
        self.transformer_after = PlusModel(
            feature_dim=feature_dim, device=device, layers=1
        ).to(device)
        ...
        self.combiner_global = CombinerSimple(
            feature_dim, feature_dim * 4, feature_dim * 8
        )
        self.combiner_local = CombinerSimple(
            feature_dim, feature_dim * 4, feature_dim * 8
        )
        self.combiner = CombinerSimple(feature_dim, feature_dim * 4, feature_dim * 8)

    def forward(
        self, ref_patch_features, text_seq_features, ref_global_feats, text_global_feats
    ):
        """
        ...
        :return: fusion feats
        """
        _, last_hidden_state, _ = self.transformer_before(
            ref_patch_features, text_seq_features
        )
...
class CombinerSimple(nn.Module):
    ...
    def __init__(
        ...
        self.text_projection_layer = self._create_projection_layer(
            clip_feature_dim, projection_dim
        )
        self.image_projection_layer = self._create_projection_layer(
            clip_feature_dim, projection_dim
        )

    def _create_projection_layer(self, input_dim, output_dim):
        return nn.Sequential(
            nn.Linear(input_dim, output_dim), nn.ReLU(), nn.Dropout(0.5)
        )
    ...
    def forward(
        ...
        raw_combined_features = torch.cat(
            (text_projected_features, image_projected_features), dim=-1
        )
...
class VisualSR(nn.Module):
    ...
    def forward(self, local_feature):
        ...
        g_emb = (
            self.embedding_global(raw_global).unsqueeze(1).repeat(1, l_emb.size(1), 1)
        )
...
class PlusModel(nn.Module):
    ...
    def forward(self, reference_features, text_features):
        ...
        inputs_embeds = torch.cat((reference_features, text_features), dim=1).to(
            self.device
        ...
        token_ref, token_text = torch.zeros([batch_size, patch_num + 1]), torch.ones(
            [batch_size, seq_num]
        )

        token_type_ids = (
            torch.cat((token_ref, token_text), dim=-1).long().to(self.device)
        )
        attention_mask = (
            torch.ones([batch_size, patch_num + seq_num + 1]).long().to(self.device)
        )
...
class Combiner2(nn.Module):
    ...
    def __init__(self, clip_feature_dim: int, projection_dim: int, hidden_dim: int):
        ...
        self.image_mlp = nn.Sequential(
            nn.Linear(clip_feature_dim, clip_feature_dim),
            nn.ReLU(),
            nn.Dropout(0.5),
        ...
        self.output_layer2 = nn.Linear(
            hidden_dim, clip_feature_dim
        )  # [batch_size, 640]
        self.dropout33 = nn.Dropout(0.5)
        self.dynamic_scalar2 = nn.Sequential(
            nn.Linear(clip_feature_dim * 2, hidden_dim),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(hidden_dim, 1),
            nn.Sigmoid(),
    ...
    def forward(
        ...
        image_sa = self.dropout11(
            F.relu(image_features * self.image_mlp(image_features))
        )
